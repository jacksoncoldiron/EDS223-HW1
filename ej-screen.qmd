...
alameda_oak_3857 <- sf::st_transform(alameda_oak, 3857)
oakland_3857     <- sf::st_transform(oakland, 3857)
osm_oak          <- tmaptools::read_osm(oakland_3857, type = "osm", ext = 1.05)

# Build the static map (with basemap first)
map1 <- tm_rgb(osm_oak) +                                                   # <- remove if no basemap
tm_shape(alameda_oak_3857) +
  tm_polygons(
    col = "P_PTSDF",
    style = "fixed", breaks = breaks_tsdf, labels = labs_tsdf,
    palette = viridis::viridis(5),
    legend.reverse = TRUE,
    alpha = 0.80,                                                   # slight transparency over basemap
    border.col = "grey70", lwd = 0.2,
    title = "Hazardous Waste Proximity (percentile)"
  ) +
  tm_shape(sf::st_boundary(oakland_3857)) + tm_lines(lwd = 1) +
